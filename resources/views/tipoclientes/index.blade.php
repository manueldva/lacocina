...
<!-- Content Header (Page header) -->
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0 text-dark">Listado T. Clientes</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="{{ route('tipoclientes.index') }}">T. Clientes</a></li>
            ...
            <div class="card">
                ...
                <form class="form-inline float-right">
                  ...
                  <input name="buscarpor" id="buscarpor" value="{{ $dato }}" class="form-control mr-sm-2" type="search" placeholder="Buscar" aria-label="Search"> 
                     <button class="btn btn-outline-success my-2 my-sm-0" type="submit"><i class="fas fa-search"></i>Buscar</button>
                      ...
                      <a class="btn btn-outline-info" href="{{ route('tipoclientes.create') }}"><i class="fas fa-plus"></i> Nuevo</a>
                      @endcan
                </form>
              ...
              <div class="card-body  p-3">
                <!--<div class="card-body p-0">-->
                         ...
                         <td>
                            <center>
                              @can('complementos.show')  
                                <a class="btn btn-sm btn-flat btn-outline-info" href="{{ route('tipoclientes.show',$tipocliente->id) }}" data-toggle="tooltip" data-placement="top" title="Ver Datos"><i class="fas fa-eye"></i> </a>
                              @endcan
                              @can('complementos.edit')  
                                <a class="btn btn-sm btn-flat btn-outline-secondary" href="{{ route('tipoclientes.edit',$tipocliente->id) }}" data-toggle="tooltip" data-placement="top" title="Editar Datos"><i class="fas fa-edit"></i> </a>
                              @endcan
                              @can('complementos.destroy')  
                                <a href="#" data-id="{{$tipocliente->id}}" class="btn btn-sm btn-flat btn-outline-danger btnDelete" data-toggle="modal" data-target="#delete"  data-toggle="tooltip" data-placement="top" title="Eliminar Registro">
                  ...
                  <div>
                    <strong>
                      <?php echo  'Mostrando ' . $tipoclientes->firstItem() . ' a ' . $tipoclientes->lastItem() . ' de ' . $tipoclientes->total() . ' registros'; ?> 
                       {{ $tipoclientes->appends(Request::only(['tipo','buscarpor']))->links() }}
                    </strong>  
                <!--</div>-->
              </div>
              </div>
...
@section('js')
  <script type="text/javascript">
    ...
    $('.btnDelete').on('click', function(){
      var tc = $(this).data('id');
      $('#frmdelete').attr('action', '{{asset('tipoclientes')}}/'+tc);
